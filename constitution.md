...
### I. Focus on Flowcharts & Framework Diagrams (流程图与框架图定位)
archviz 的核心定位是**面向 AI 智能体的克制化流程图与框架图（如系统架构图、概念模型、状态机、数据流、关系矩阵等）设计规范与可视化工具包**。传统的数值统计图表（如柱状图、折线图等）应退居次要位置或由通用图表库处理。所有图面设计必须服务于结构、流程和概念关系的高清晰度表达，通过严谨的网格和信息层级揭示逻辑，严禁无序拼凑、彩虹配色和过度修饰。

### II. Restraint as Dial, Not Default
...
### V. Color Restraint & User-Driven Iteration
...
### VI. Single Artifact Convergence
...
### VII. Gotchas as Living Single Source of Truth (Highest Value)
...
- Mixed types: never combine >2 in one diagram unless deliberate "combined page" demonstration (see Principle VI and examples/us-flows.html). Split otherwise.
